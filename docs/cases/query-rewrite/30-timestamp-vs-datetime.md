...
## TIMESTAMP 存 UTC：国内报表订单"消失"的时区陷阱
...
```bash
# 默认在 MySQL 8.0 上运行
./scripts/run-case.sh 30-timestamp-vs-datetime

# 在 MySQL 5.7 上运行（对比）
./scripts/run-case.sh 30-timestamp-vs-datetime --ver 5.7

# 跳过造数据重跑
./scripts/run-case.sh 30-timestamp-vs-datetime --no-seed
```
