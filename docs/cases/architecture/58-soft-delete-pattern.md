...
## 软删除 20% 数据后查询变慢：deleted_at 索引方案
...
```bash
# 默认在 MySQL 8.0 上运行
./scripts/run-case.sh 58-soft-delete-pattern

# 在 MySQL 5.7 上运行（对比）
./scripts/run-case.sh 58-soft-delete-pattern --ver 5.7

# 跳过造数据重跑
./scripts/run-case.sh 58-soft-delete-pattern --no-seed
```
