...
## 50 万行一条 UPDATE：CPU 飙升、主从延迟 30 秒
...
```bash
# 默认在 MySQL 8.0 上运行
./scripts/run-case.sh 79-batch-update

# 在 MySQL 5.7 上运行（对比）
./scripts/run-case.sh 79-batch-update --ver 5.7

# 跳过造数据重跑
./scripts/run-case.sh 79-batch-update --no-seed
```
